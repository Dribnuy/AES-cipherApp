...
function expandKey(key: number[]): number[][] {
  ...
  for (let i = 4; i < 44; i++) {
    ...
    expandedKey[i] = expandedKey[i-4].map((b, j) => b ^ temp[j]);
  }
  return expandedKey;
}

function getRoundKeys(expandedKey: number[][]): number[][][] {
  const roundKeys: number[][][] = [];
  for (let round = 0; round < 11; round++) {
    const roundKey: number[][] = [[], [], [], []];
    for (let col = 0; col < 4; col++) {
      for (let row = 0; row < 4; row++) {
        roundKey[row][col] = expandedKey[round * 4 + col][row];
      }
    }
...
function aesCipher(input: number[], key: number[]): number[] {
  const roundKeys = getRoundKeys(expandKey(key));
  const state: number[][] = [[], [], [], []];
  ...
  const output: number[] = [];
  for (let i = 0; i < 16; i++) output[i] = state[i % 4][Math.floor(i / 4)];
  return output;
}

function aesInvCipher(input: number[], key: number[]): number[] {
  const roundKeys = getRoundKeys(expandKey(key));
  const state: number[][] = [[], [], [], []];
  for (let i = 0; i < 16; i++) state[i % 4][Math.floor(i / 4)] = input[i];
...
function pkcs7Unpad(data: Buffer): Buffer {
  ...
  if (padding > 16 || padding === 0 || padding > data.length) return data;
